...
- (void)layoutSubviews
{
    ...
    float scaledStart = 0, scaledProgress = 0, leftCropProgress = 0, rightCropProgress = 0, scaledEnd = 1, scaledWidth = 1;
    if (self.cachedEndSamples > self.cachedStartSamples) {
        ...
        leftCropProgress = ((float)self.cropStartSamples-self.zoomStartSamples)/(self.zoomEndSamples-self.zoomStartSamples);
        rightCropProgress = ((float)self.cropEndSamples-self.zoomStartSamples)/(self.zoomEndSamples-self.zoomStartSamples);
    }
...
- (void)renderAsset
{
    ...
    [FDWaveformView sliceAndDownsampleAsset:self.asset track:self.assetTrack startSamples:renderStartSamples endSamples:renderEndSamples targetSamples:widthInPixels done:^(NSData *samples, NSInteger sampleCount, Float32 sampleMax) {
